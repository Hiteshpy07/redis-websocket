...
const textInputRef = useRef(null);
  ...
  // Random color assigner per socket connection session
  ...
  // Main rendering utility function for brush, shapes, and text
  const drawOnCanvas = (type, x1, y1, x2, y2, strokeColor, textValue = '') => {
...
const submitTextOverlay = () => {
  ...
  const startDrawing = ({ nativeEvent }) => {
   ...
   if (activeTool === 'text') {
    ...
    if (canvas && ctx) {
      // Freeze snapshot buffer data state
      ...
      <div className="w-full h-full flex flex-col bg-gray-950 relative">
        ...
        <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-900 z-10">
          <div className="flex items-center gap-4">
            ...
            {/* Shapes & tools ribbon container */}
            <div className="flex bg-gray-950 border border-gray-800 p-1 rounded-md gap-1">
              {[
                { id: "brush", label: <FaPaintbrush /> },
                { id: "rect", label: <FaRegSquare /> },
                { id: "circle", label: <FaRegCircle /> },
                { id: "line", label: <TbStrokeStraight /> },
                { id: "text", label: <IoText /> }
              ].map((tool) => (
                <button
                  key={tool.id}
                  type="button"
